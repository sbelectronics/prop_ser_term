...
idle                    jmpret  rxcode,txcode         'run chunk of tx code, then return
        ...
        if_nz           sub     t1,cnt
        if_nz           cmps    t1,#0           wc
        if_nz_and_c     andn    outa,rxledmask
        if_nz_and_c     mov     rxledcnt, #0
...
transmit                jmpret  txcode,rxcode         'run chunk of rx code, then return
        ...
        if_nz           sub     t1,cnt
        if_nz           cmps    t1,#0           wc
        if_nz_and_c     andn    outa,txledmask
        if_nz_and_c     mov     txledcnt, #0

                        jmpret  txcode,rxcode         'run chunk of rx code, then return

                        mov     t1,rxledcnt     wz    'rx led
        if_nz           sub     t1,cnt
        if_nz           cmps    t1,#0           wc
        if_nz_and_c     andn    outa,rxledmask
        if_nz_and_c     mov     rxledcnt, #0
                        ...
                        mov     t1,par                'handle rts flag
                        rdlong  t2,t1
                        add     t1,#1 << 2
                        rdlong  t3,t1
                        sub     t2,t3           wc
        if_c            add     t2,#BUFFER_LENGTH
                        mov     t3,#0
                        cmp     t2,#BUFFER_MIN  wc
        if_c            andn    outa,rtsmask
                        cmp     t2,#BUFFER_MAX  wc
        if_nc           or      outa,rtsmask

                        jmpret  txcode,rxcode         'run chunk of rx code, then return

                        test    rxtxmode,#%001  wz    'check cts
                        test    ctsmask,ina     wc
        if_z_eq_c       jmp     #transmit
...
ledticks                long    0
rxledmask               long    1 << RX_LED_PIN
txledmask               long    1 << TX_LED_PIN

rtsmask                 long    1 << UART_RTS_PIN
ctsmask                 long    1 << UART_CTS_PIN
